...
vec4 render(vec2 st, vec2 mouse) {
    ...
    // Return with transparency so background shows through
    return vec4(color, shape * dimming * vignette);
}
...
void main() {
    ...
    vec4 result = render(st, mouse);
    
    // Write out the result with alpha for transparent layering
    gl_FragColor = result;
}
`;
...
export default function GeometricBlurMesh({ className = "" }: { className?: string }) {
    ...
    useEffect(() => {
        ...
        // Set clear color to transparent so DottedSurface shows through
    ...
    return (
        <div ref={containerRef} className={`absolute inset-0 w-full h-full cursor-pointer overflow-hidden pointer-events-auto z-10 bg-transparent ${className}`}>
            <canvas 
                ref={canvasRef}
                className="w-full h-full bg-transparent mx-auto opacity-80"
            />
            <div className="absolute top-[80px] right-2 transform pointer-events-none text-right">
                <div className="text-white/40 text-xs font-light tracking-widest">
                    {shapes[currentShape]?.toUpperCase()}
                </div>
                <div className="text-white/20 text-[10px] font-light mt-1 tracking-wide">
                    CLICK BG TO SWITCH
                </div>
            </div>
        </div>

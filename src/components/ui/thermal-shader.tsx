...
const ThermalEffect: React.FC<ThermalEffectProps> = ({
  ...
  height = "100%",
  className = "",
  logoUrl = "https://raw.githubusercontent.com/dalim-in/dalim/refs/heads/main/apps/ui/public/apple.png",
  parameters = {},
}) => {
  ...
  useEffect(() => {
    if (engineRef.current && parameters) {
      Object.entries(parameters).forEach(([key, value]) => {
        engineRef.current?.setParameter(key as keyof EffectParameters, value as number)
      })
    }
  }, [parameters])
  ...
  useEffect(() => {
    ...
    const initEngine = async () => {
      try {
        ...
        await engine.init()
        // Apply initial custom parameters
        if (parameters) {
          Object.entries(parameters).forEach(([key, value]) => {
            engine.setParameter(key as keyof EffectParameters, value as number)
          })
        }
...
class ThermalEffectEngine implements Disposable {
  ...
  private setupInteractionManager(): void {
    const hitContainer = (this.container.closest('body') || this.container) as HTMLElement
  ...
  private updateMeshTransform(): void {
    ...
    if (this.heatMesh) {
      this.heatMesh.scale.set(frustumWidth * scale, frustumHeight * scale, 1)
    }
  }
...
class DrawRenderer implements Disposable {
	...
	constructor(size = DRAW_RENDERER.TEXTURE_SIZE, options: DrawRendererOptions = {}) {
		...
		const rtOpts: THREE.RenderTargetOptions = {
			...
			colorSpace: (THREE as any).LinearSRGBColorSpace || THREE.NoColorSpace,
			depthBuffer: false,
			stencilBuffer: false,
...
void main() {
    ...
    float noiseAnim = smoothNoise(uv * 3.0 + vec2(time * 0.2, time * 0.25));
    float noiseAnim2 = smoothNoise(uv * 7.0 - vec2(time * 0.15, time * 0.1));
    float timeAnim = (noiseAnim * 0.6 + noiseAnim2 * 0.4);
    
    // Base heat should be present everywhere to ensure visibility
    heatDraw += baseAnimationLevel * timeAnim;
    
    // Add interaction heat
    float map = pow(heatDraw, power);

    vec3 final = gradient(map);
    final = saturation(final, colorSaturation);

    // Apply main visibility and intensity

...
const Helper = {
  ...
  uniform2DFragmentVaryingShader: `
    ...
    void main(){
      // Calculate a time-based blend parameter between 0 and 1
      float t = v_color.x + u_tick;
      t = abs(t - floor(t)); 
      
      // Use sine wave to smoothly oscillate between red and blue
      float blend = (sin(t * 6.28318) * 0.5) + 0.5;
      
      // Colors from INSCRIBE hero text
      vec3 colorRed = vec3(1.0, 0.294, 0.294);  // #FF4B4B
      vec3 colorBlue = vec3(0.294, 0.482, 1.0); // #4B7BFF
      
      vec3 finalColor = mix(colorRed, colorBlue, blend);
      
      // Fade out based on particle lifetime (v_color.y)
      gl_FragColor = vec4(finalColor * v_color.y, 1.0);
    }
  `
};
...
const ParticleCanvas = ({ maxParticles = 1000, particleSizeMin = 2, particleSizeMax = 5, speedScale = 2 }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      // Offset cursor particle emission point slightly to the right to perfectly align with custom dot
      dimensionsRef.current.cx = e.clientX + 4;
      dimensionsRef.current.cy = e.clientY + 2; 
    };
